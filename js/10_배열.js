...
function selectMenu() {
  ...
  // 점심 메누로 초기화된 배열 생성
  const menus = ["라면" + "김밥", "제육볶음", "치즈돈까스",
                "샐러드", "맘스터치", "알탕", "돼지국밥", "닭갈비", "형제네",
                "KFC", "냉면", "콩국수", "쭈꾸미", "초밥", "쌀국수", "김치찜"]
  
  const randomNumber = menus[Math.floor(Math.random() * menus.length)];
  // menus 배열에서 랜덤으로 한개 뽑아서
  // 화면에 #menuResult에 출력하기

  console.log(randomNumber);

  menuResult.innerText = randomNumber;
}
