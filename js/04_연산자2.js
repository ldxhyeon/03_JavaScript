...
function oddEvenCheck() {
  ...
  // #input1 요소에 작성 된 값을 얻어와
  // Number()를 이용해 숫자로 변환 후
  // value 변수에 대입(저장)
  const value = Number(input1.value);

  // 2로 나누었을때 나머지가 0 === 짝수(2의 배수)
  // 2로 나누었을때 나머지가 1이 아닌경우 === 짝수
  
  // 2로 나누었을때 나머지가 1이면 === 홀수
  // 2로 나누었을때 나머지가 0이 아닌 경우 === 홀수
...
function multipleCheck() {
  ...
  // 10은 5의 배수? -> 10을 5로 나눴을때 나머지가 0인지 확인
...
function compoundCheck() {
  ...
  console.log(`number += 2 : ${number}`); // 120

  number -= 60; // number = number - 60;
  console.log(`number -= 2 : ${number}`); // 60

  number *= 3; // number = number * 3;
  console.log(`number *= 2 : ${number}`); // 180

  number /= 10; // number = number / 10;
  console.log(`number /= 2 : ${number}`); // 18
...
function andCheck() {
  ...
  // 13은 4부터 15 사이의 숫자가 맞는가 ?

  const bool4 = (4 <= 13) && (13 <= 15);
  ...
  console.log(`13은 4부터 15 사이의 숫자가 맞는가 ? ${bool4}`)
...
function orCheck() {
  ...
  console.log(`4는 10을 초과하거나 짝수인가 ? ${bool1}`);
   ...
   const boo2 = (age < 18) || (name === "김길동");

   console.log(`나이가 18세 미만 이거나, 이름이 "김길동"인 경우 ${boo2}`)
