...
// (에러) number3 in not defined
// -> number3라는 변수가 정의되지 않았따.
// == number3 선언 안됨 / 존재 X
...
function letCheck() {
  ...
  // (새로운 값 대입 불가)
function constCheck() {
  const nationalCode = 82; // 국가코드

  console.log("국가 코드 : ",nationalCode);

  nationalCode = 1; // const에 값을 대입해도 에러 발생 X
...
function scopeTest() {
  ...
  function scopeTest2() {
    ...
    if(1 == 1) {
      const loacal1 = "if 지역 변수1";

      console.log("if 내부에서 global : ",global1);
      
      //넓은 범위(함수)에서 선언된 변수는
      //좁은 범위(함수 내 if)에서 사용 가능!
      console.log("if -> fn1 : ",fn1);
      console.log("if -> local1 : ",loacal1);
